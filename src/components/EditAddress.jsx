...
const EditAddressPage = ({ id }) => {
  ...
  useEffect(() => {
    const fetchAddressData = async () => {
      try {
        setIsLoading(true);
        const addressData = await addressApi.getAddressById(addressId);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const { isValid, errors } = validateAddress(data);
      ...
      // If the address is set as default, make API call to set it as default
      if (data.isDefault) {
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-64">
        ...
        <span className="ml-2">Loading address details...</span>
      </div>
    );
  }
